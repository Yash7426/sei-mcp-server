...
import { config as envConfig } from "../core/config.js";

// Use environment variables from env config
...
app.post("/messages", (req: Request, res: Response) => {
  ...
  // Skip sessionId check for the /balance endpoint
  if (req.path !== "/balance" && req.path!=="/transfer") {
    if (!sessionId) {
    ...
    console.error(`Handling message for /balance & /transfer`);
  }
});

import { getBalance } from "../core/services/balance.js";
import { transferSei } from "../core/services/transfer.js"
import { getTransaction } from "../core/services/transactions.js";
// @ts-ignore
app.post("/balance", async (req: Request, res: Response) => {
  try {
    ...
    const balanceDetails = await getBalance(address,network);


    if (!balanceDetails) {
      return res
        .status(400)
        .json({ success: false, error: "Balance not found" });
    }

    console.error("Balance :", balanceDetails);
    return res.json({address, network, balance: balanceDetails.sei });

  } catch (error) {
    console.error("Error in /balance", error);
    return res
      .status(500)
      .json({ success: false, error: (error as any)?.message || error });
  }
});
...
app.post("/transfer", async (req: Request, res: Response) => {
  try {
    ...
    if (!to || !network || !amount) {
      return res
        .status(400)
        .json({ success: false, error: "Missing required fields" });
    }
    const transferDetails = await transferSei(to,amount,network);
    ...
    console.error("Transfer :", transferDetails);
    const details = await getTransaction(transferDetails,network)
    console.log(details)
    return res.json({
    network,
    sender: details.from,
    ...
    console.error("Error in /evm/tx/verify:", error);
    return res
      .status(500)
      .json({ success: false, error: (error as any)?.message || error });
  }
...
const httpServer = app
  .listen(PORT, HOST, () => {
    ...
    console.error(
      `Messages endpoint: http://${HOST}:${PORT}/messages (sessionId optional if only one connection)`
    );
